...
from neuroglobe.projections.definitions import CONFIGS_DIR, PROCESSED_DATA_DIR
...
def filter_csv(input_csv_path: str = None):
    ...
    required = {"acronym", "value"}
    missing = required - set(df.columns)
    if missing:
        log.error("Input CSV is missing columns: %s", sorted(missing))
        return False
    duplicates = sorted(df.loc[df["acronym"].duplicated(), "acronym"].unique())
    if duplicates:
        log.error(
            "Input CSV has duplicate acronyms; regenerate aggregation first: %s",
            duplicates[:10],
        )
        return False
...
def main() -> int:
    ...
    return 0 if filter_csv(csv_arg) else 1
