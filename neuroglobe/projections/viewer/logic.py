...
def process_csv_data(file_path: str, colormap_name="viridis") -> CSVProcessResult:
    try:
        df = pd.read_csv(file_path)
        # Check columns (support the new optional is_seed column)
        if "acronym" not in df.columns or "value" not in df.columns:
            raise CSVDataError("CSV must have 'acronym' and 'value' columns")
        if df.empty:
            raise CSVDataError("CSV contains no rows.")
        df["acronym"] = df["acronym"].astype(str).str.strip()
        if (df["acronym"] == "").any():
            raise CSVDataError("CSV contains an empty acronym.")
        duplicated = sorted(df.loc[df["acronym"].duplicated(), "acronym"].unique())
        if duplicated:
            raise CSVDataError(
                f"CSV contains duplicate acronyms: {duplicated[:10]}"
            )
            
        # If is_seed does not exist (old CSVs), create it as False
        if 'is_seed' not in df.columns:
            df['is_seed'] = False
        elif df["is_seed"].dtype != bool:
            normalized = df["is_seed"].astype(str).str.strip().str.lower()
            allowed = {"true", "false", "1", "0"}
            if not set(normalized).issubset(allowed):
                raise CSVDataError("is_seed must contain only true/false values.")
            df["is_seed"] = normalized.isin({"true", "1"})
            
    except CSVDataError:
        raise
    except Exception as e:
        log.error(f"CSV Load Error: {e}")
        raise CSVDataError(f"Could not load CSV: {e}") from e
    ...
    for column in set(present_cols + ["value"]):
        try:
            df[column] = pd.to_numeric(df[column], errors="raise")
        except Exception as error:
            raise CSVDataError(f"Column {column!r} must be numeric.") from error
    ...
    if not targets_df.empty and present_cols:
        ...
        v_min = 0.0 
        if not pd.notna(v_max) or float(v_max) <= 0:
            v_max = 1.0
        norm = mcolors.Normalize(vmin=v_min, vmax=float(v_max))
    else:
        v_min, v_max = 0.0, 1.0
    ...
    try:
        ...
        raise CSVDataError(f"Unknown colormap: {colormap_name}") from error
    ...
    return CSVProcessResult(results, float(v_min), float(v_max))
