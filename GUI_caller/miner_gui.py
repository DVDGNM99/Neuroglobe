...
class MinerApp(ctk.CTk):
    ...
    def run_script(self, script_name):
        ...
        if not success:
            self.log_console("[ERROR] Aborting run due to invalid configuration.")
            return

        script_path = PROJECT_ROOT / "src" / "miner" / script_name
        if not script_path.exists():
            self.log_console(f"[ERROR] Script not found: {script_path}")
            return
        
        self.log_console(f"--- Running {script_name} ---")
        ...
        def task():
            try:
                ...
                if stderr_output:
                     self.after(0, self.log_console, f"[STDERR] {stderr_output}")

                self.after(0, self.log_console, f"--- Finished {script_name} ---")
    ...
    def run_script(self, script_name, output_widget=None):
        ...
        def task():
            try:
                ...
                stderr_output = process.stderr.read()
                if stderr_output:
                     self.after(0, self.log_to_widget, output_widget, f"[STDERR] {stderr_output}")

                self.after(0, self.log_to_widget, output_widget, f"--- Finished {script_name} ---")
            except Exception as e:
